...
> - **【未確定】** 次フェーズで詰める事項
> - **【会社からの修正】** 社内 exe 配布・共有 DB 運用開始後に会社（利用者側）から挙がった変更。§8.11 以降で管理する（**実装済み** / **未実装** は各節の見出しを参照）
...
### 8.11 共有ファイルサーバー配布：データ根の相対パス化【2026-06-18 **会社からの修正**】

> **位置づけ**: **§8.1 以降の社内開発フェーズとは別枠** の要件。Portal を exe 化して社内 PC に配布し、共有フォルダ上の `portal-master.db` を複数端末で使う運用を開始した結果、会社（利用者側）から修正依頼があった事項。  
>
> | 区分 | 節 | 状態（2026-06-19） |
> |------|-----|-------------------|
> | 第1弾 | §8.11.4〜8.11.6（ヒーロー背景・生産ボード DB・添付） | **実装済み**（2026-06-18） |
> | 第2弾 | §8.11.11（CSV/Excel インポート用テンプレート） | **未実装**（2026-06-19 要件確定） |
>
> 第1弾の実装: `src/main/db/dataRoot.ts`・`portalAssets.ts`・`seisanBoardPath.ts`、`settings.handler.ts`、`seisanConnection.ts`、`projectFiles.repo.ts` ほか。

#### 8.11.1 会社からの修正：経緯・課題

| 項目 | 内容 |
|------|------|
| **運用形態** | 開発 PC で `npm run dist` したインストーラーを社内配布。各 PC に Portal をインストール |
| **共有 DB** | 全 PC が同じ `portal-master.db` を選択（例: `L:\YS-application\portal\data\portal-master.db`） |
| **正常に動く範囲** | ログイン、ポータル本体、ログインアプリ（操作者・マスタ参照） |
| **不具合 1** | **ヒーロー背景**（ホーム LP の会社名エリア背景）が PC によって表示されない |
| **不具合 2** | **生産ボード**の DB パスが PC ごとに異なり、共有しているはずの案件データが表示されない |
| **不具合 3** | 生産ボード設定から **CSV インポート用 Excel テンプレート（`format.xlsx`）** をダウンロードできない（インストール版で `resources/format.xlsx` の参照パスが合わない） |

**会社側の要望（要約）**

- `portal-master.db` の選択先は **全員同じ** に揃えられる
- その前提なら、**`portal-master.db` があるフォルダを基準とした相対パス** でファイル・サテライト DB を参照すれば、端末間で共有できるのではないか

#### 8.11.2 現行実装とのギャップ（原因整理）

| 対象 | 現行の保存・解決方法 | 共有運用で起きること |
|------|----------------------|----------------------|
| **ポータル DB パス** | 各 PC の `%APPDATA%\portal\portal-config.json` に **絶対パス** を記憶 | 表記は `L:\...` と `\\server\...` でバラけるが、**同じ物理ファイルを指せばログインは可能** |
| **ヒーロー背景** | 中央 DB の `app_settings`（`company.homeHeroBackgroundPath`）に **画像の絶対パス** を保存。表示時に **その PC 上で `existsSync`** | 管理者 PC の `C:\Users\...` や、他 PC で未割り当ての `L:` など **端末依存パス** だと背景が出ない |
| **生産ボード DB** | 原則 `{portal-master.db と同じフォルダ}\seisan-board.db`。ただし **各 PC の `seisan-board-override.json`** で別パスに上書き可能 | override や「DB を選択」で **PC ごとに別 DB** を見ると、案件データが分断される |
| **生産ボード添付** | `project_files.file_path` に **絶対パス** を保存（第1弾以前） | 共有運用では添付参照も **データ根からの相対パス** が必要 → **第1弾で実装済み** |
| **CSV/Excel インポート用テンプレ** | リポジトリ同梱 `resources/format.xlsx` を **アプリ内パス**（`app.asar` / 誤った `process.resourcesPath`）から読む | インストール版でファイル未検出。**共有フォルダのデータ根に置く方式へ変更**（§8.11.11） |
| **図面ライブラリ** | `{中央 DB フォルダ}\drawing-library.db` ＋ 実ファイルは **相対パス**（参考実装あり） | **既に相対パス方式** — 本修正の **お手本** とする |

#### 8.11.3 データ根（Data Root）の定義【会社修正・決定】

| 用語 | 定義 |
|------|------|
| **データ根** | 接続中の **`portal-master.db` ファイルの親ディレクトリ**（`dirname(portal-master.db)`） |
| **共有データフォルダ** | 社内ファイルサーバー上の 1 フォルダ。全 PC から読み書き可能であること |

**例（会社運用）**

```
L:\YS-application\portal\data\          ← データ根（UNC でも可）
├── portal-master.db                    ← 中央 DB（全員同じファイル）
├── seisan-board.db                     ← 生産ボード（サテライト）
├── drawing-library.db
├── process-management.db
├── parts-tracker.db
├── assets\                             ← ポータル共通の静的アセット（§8.11.4）
│   └── hero-background.jpg             ← ヒーロー背景（例）
└── resources\                          ← 共有テンプレート等（§8.11.11・新設）
    └── format.xlsx                     ← 案件 CSV インポート用 Excel テンプレ
```

> **命名注意**: データ根配下の `resources/` は、**Electron ビルド同梱用** のリポジトリ `portal/resources/`（`branding/`・`tools/` 等）とは **別物**。共有データフォルダ側は **業務テンプレート置き場** として扱う。

- データ根は **DB 接続のたびに動的に決定** する（固定ドライブ文字や `%APPDATA%` 固定ではない）。
- 各 PC が `portal-master.db` を **異なる表記**（`L:\...` / UNC）で開いても、**同じ物理フォルダ** を指していれば、相対パス解決の結果は一致する。

#### 8.11.4 要件：ヒーロー背景【会社修正・**実装済み 2026-06-18**】

| # | 要件 | 優先 |
|---|------|------|
| H-1 | 管理画面で背景画像を選んだとき、**データ根配下の `assets/` にコピー** し、中央 DB には **データ根からの相対パス**（例: `assets/hero-background.jpg`）のみ保存する | 必須 |
| H-2 | 表示時は `join(データ根, 相対パス)` で解決し、`file://` URL を生成する | 必須 |
| H-3 | **後方互換**: 既存 DB に絶対パスが入っている場合は、従来どおり絶対パスとして解決を試みる（移行期間） | 必須 |
| H-4 | 画像ファイルは **共有フォルダ上** に置くため、全 PC から読み取り可能な権限が必要（運用手順で明記） | 運用 |
| H-5 | モットー・会社名は従来どおり中央 DB の `app_settings` のみ（変更なし） | — |

**非ゴール（本修正）**

- 背景画像の CDN 配信、複数解像度の自動生成

#### 8.11.5 要件：生産ボード DB【会社修正・**実装済み 2026-06-18**】

| # | 要件 | 優先 |
|---|------|------|
| S-1 | 中央 DB 接続後、生産ボード DB は **常に** `{データ根}/seisan-board.db` を使用する（**共有運用の既定**） | 必須 |
| S-2 | 各 PC の `%APPDATA%\portal\seisan-board-override.json` による **端末ローカル上書きを廃止** するか、共有運用モードでは **無視** する | 必須 |
| S-3 | 生産ボード設定画面の「DB を選択」は、**共有運用では `{データ根}/seisan-board.db` を案内** するか、管理者向けに **データ根固定** とし、別パス指定 UI を縮退する | 推奨 |
| S-4 | `seisan-board.db` が存在しない場合の挙動を定義する（**共有フォルダ上で 1 回だけ作成**し、以降全 PC で共有。空 DB の PC ごと新規作成は禁止） | 必須 |
| S-5 | 生産ボード設定に表示する「現在の DB パス」は **解決後の実パス** を表示してよいが、**データの正は常にデータ根配下** であることを UI 文言で示す | 推奨 |

#### 8.11.6 要件：生産ボード添付ファイル【会社修正・**実装済み 2026-06-18**】

| # | 要件 | 優先 |
|---|------|------|
| F-1 | 新規添付の `project_files.file_path` は **`seisan-board.db` の親ディレクトリ（= データ根）からの相対パス** で保存する | 必須 |
| F-2 | 読み取り・削除・ダウンロード時は相対パスをデータ根から解決する | 必須 |
| F-3 | 既存レコードの **絶対パス** は読み取り時にそのまま解決を試みる（後方互換） | 必須 |
| F-4 | 一括移行スクリプトの要否は **実装フェーズで判断**（データ量・既存運用次第） | 未確定 |

#### 8.11.11 要件：CSV/Excel インポート用テンプレート（`format.xlsx`）【会社修正・**未実装 2026-06-19**】

> **背景**: 生産ボード設定 › 「CSVインポート用フォーマット（Excel）」からテンプレートを保存する機能がある。開発環境ではリポジトリ同梱の `resources/format.xlsx` を参照できるが、**インストール版（exe）ではパス解決に失敗** し、「CSVインポート用フォーマット (format.xlsx) が見つかりません」と表示される（不具合 3）。  
> **方針**: ヒーロー背景（§8.11.4）と同様、**データ根配下** に正本を置き、全 PC から同じファイルを参照する。アプリ同梱ファイルは **フォールバック** とする。

##### 8.11.11.1 現行実装の問題点

| 項目 | 内容 |
|------|------|
| **参照コード** | `src/main/modules/seisan-import/seisan-import.handler.ts` の `resolveFormatXlsxPath()` |
| **packaged 時のパス** | `join(process.resourcesPath, "resources", "format.xlsx")` → 実際には `…\resources\resources\format.xlsx` を探して失敗しうる |
| **開発時** | `join(app.getAppPath(), "resources", "format.xlsx")` は動作する |
| **CSV テンプレ** | `buildCsvImportTemplateContent()` で **メモリ生成** のため影響なし（本節の対象外） |

##### 8.11.11.2 正本の配置

| 項目 | 定義 |
|------|------|
| **正本パス** | `{データ根}/resources/format.xlsx` |
| **データ根** | §8.11.3 と同じ（`dirname(portal-master.db)`） |
| **ファイル内容** | リポジトリ `resources/format.xlsx` と **同一**（1 行目ヘッダーは `csvImportFormat.ts` の `CSV_IMPORT_HEADERS` と整合） |
| **更新方針** | 列定義変更時は **共有フォルダ上の 1 ファイル** を管理者が差し替え。全 PC で同一テンプレを参照 |

##### 8.11.11.3 解決優先順位（実装要件）

`resolveFormatXlsxPath()`（または同等ヘルパー）は、中央 DB 接続後に次の順で存在確認する。

| 優先 | パス | 用途 |
|------|------|------|
| 1 | `{データ根}/resources/format.xlsx` | **共有運用の正本** |
| 2 | アプリ同梱 `format.xlsx` | データ根に未配置の **初回・オフライン・フォールバック** |

**優先 2 の同梱パス**（packaged / 開発の両方で正しく解決すること）:

| 環境 | 解決先（例） |
|------|----------------|
| 開発 | `join(app.getAppPath(), "resources", "format.xlsx")` |
| インストール版 | `join(process.resourcesPath, "format.xlsx")` または electron-builder の `extraResources` 配置に合わせた **1 段** のパス（`resources/resources` の二重参照を禁止） |

##### 8.11.11.4 初回シード（データ根への配置）

共有フォルダに `resources/format.xlsx` が **存在しない** 場合の挙動を定義する。

| # | 要件 | 優先 |
|---|------|------|
| T-1 | 管理者が「CSVインポート用フォーマットをダウンロード」実行時、**優先 2**（同梱）から読めれば **ユーザー指定先へ保存** できる（現行 UI のまま） | 必須 |
| T-2 | ダウンロード成功時、**任意で** `{データ根}/resources/` を作成し、同梱 `format.xlsx` を **データ根へコピー（シード）** する | 推奨 |
| T-3 | シード後は **優先 1** から配布され、以降の Portal 更新（exe 差し替え）でも **列定義付きテンプレはデータ根側が正** となる | 推奨 |
| T-4 | データ根への書き込み権限がない PC では、**同梱フォールバックのみ** でダウンロード可能とし、エラーにしない | 必須 |
| T-5 | 運用手順として、初回セットアップ時に管理者が **手動で** `L:\…\data\resources\format.xlsx` を配置してもよい（リポジトリからコピー可） | 運用 |

> **T-2 の判断**: 自動シードを入れるかは実装フェーズで決定。最低限 **優先 1 → 2 の解決** と **手動配置** で受け入れ可能とする。

##### 8.11.11.5 IPC・UI

| 項目 | 要件 |
|------|------|
| **IPC** | 既存 `seisan-import:downloadFormat`（`SEISAN_CHANNELS.import.downloadFormat`）を維持。レスポンス形式 `{ success, data \| error }` 不变 |
| **権限** | 現行どおり `assertCanViewApp("seisan-board")` |
| **設定 UI** | `SettingsGeneralPage` の説明文を更新し、正本が **`{データ根}/resources/format.xlsx`**（共有フォルダ）であることを示す。「アプリ同梱のみ」という表現はやめる |
| **エラーメッセージ** | 優先 1・2 とも不存在の場合のみ失敗。メッセージに **データ根への配置手順**（`resources/format.xlsx`）を含める |

##### 8.11.11.6 非ゴール

- Excel テンプレの **アプリ内編集 UI**（列追加を GUI で行う等）
- テンプレの **バージョン管理 DB 化**（ファイル 1 本運用で足りる）
- CSV インポート **処理ロジック** の変更（列定義は `csvImportFormat.ts` のまま）

##### 8.11.11.7 実装時の変更対象（参考・実装は別タスク）

| ファイル | 変更内容（予定） |
|----------|------------------|
| `src/main/modules/seisan-import/seisan-import.handler.ts` | `getDataRoot()` 連携、優先 1→2 解決、同梱パス修正、任意シード |
| `src/main/db/dataRoot.ts` | 必要なら `resources/` 用ヘルパー（例: `resolveDataRootResource()`） |
| `src/renderer/.../SettingsGeneralPage.tsx` | 説明文案の更新 |
| 共有フォルダ | 初回 `resources/format.xlsx` 配置（運用） |

#### 8.11.7 変更しないもの（スコープ外）

| 項目 | 理由 |
|------|------|
| **各 PC の `portal-config.json`** | 「前回開いた `portal-master.db` のパス」記憶は端末ごとでよい。共有 DB の **物理パス表記**（`L:` / UNC）の統一は **運用手順** で対応 |
| **Pixo Converter の temp** | 引き続き `%APPDATA%` 配下（端末ローカル作業領域） |
| **中央 DB 内のマスタ・操作者データ** | 既に共有 DB で同期されているため変更不要 |
| **コード署名・インストーラー配布手順** | 本節の対象外 |

#### 8.11.8 運用手順（会社向け・実装とセットで周知）

1. 共有フォルダに **§8.11.3 の構成** を用意する（初回は管理者 PC で中央 DB 作成 → 同フォルダにサテライト DB を配置）。
2. 全 PC で Portal 初回起動時、**同じ `portal-master.db`** を選択する（可能なら **UNC パスで統一**）。
3. ヒーロー背景は、管理者が **管理 › 会社情報** から画像を設定する（`assets/` にコピーされる）。
4. 生産ボード設定で **ローカルパスの DB を選ばない**（データ根固定）。
5. **CSV インポート用 Excel テンプレ**（§8.11.11）: 共有フォルダの `{データ根}/resources/format.xlsx` を配置する。未配置の場合は生産ボード設定からダウンロード（実装後は同梱からの初回シードも可）。
6. 問題 PC では `%APPDATA%\portal\seisan-board-override.json` の有無を確認する（第1弾以前の移行期間用。現在は override 無効）。

#### 8.11.9 受け入れ基準（会社修正）

**第1弾（§8.11.4〜8.11.6）— 実装済み**

- [x] PC-A・PC-B の両方で **同じ共有 `portal-master.db`** を接続し、**同じヒーロー背景** がホームに表示される。
- [x] PC-A で登録した生産ボード案件が、PC-B から **同じ内容で参照** できる（データ根配下の **同一 `seisan-board.db`**）。
- [x] ヒーロー背景の `app_settings` 値が **相対パス**（例: `assets/...`）になっている（新規設定時）。
- [x] `seisan-board-override.json` が存在する PC でも、**データ根の `seisan-board.db`** が使われる（override 廃止）。
- [x] 図面ライブラリの既存相対パス方式と **整合** した解決ロジック（`dataRoot.ts`）を用いる。
- [x] 既存 DB（絶対パス入り）を開いても **クラッシュせず**、可能な範囲で表示継続（後方互換）。

**第2弾（§8.11.11）— 未実装**

- [ ] **インストール版（exe）** の生産ボード設定から、CSV インポート用 Excel テンプレを **エラーなくダウンロード** できる。
- [ ] `{データ根}/resources/format.xlsx` が存在する場合、ダウンロード元は **常にそのファイル**（全 PC で同一内容）。
- [ ] データ根に未配置でも、**アプリ同梱** からダウンロードできる（フォールバック）。
- [ ] packaged 時に `resources/resources/format.xlsx` のような **誤パス参照をしない**。
- [ ] 設定画面の説明が **データ根配下の正本** を示している。

#### 8.11.10 未確定事項

| # | 論点 | 推奨 |
|---|------|------|
| 1 | override ファイルの **完全削除** vs **共有モード時のみ無視** | 共有モードフラグなしで **override 廃止**（シンプル） |
| 2 | `seisan-board.db` 初回不存在時 | データ根に **明示作成**（管理者操作 1 回）。黙って PC ローカルに作らない |
| 3 | 添付ファイルの **既存絶対パス一括移行** | 必要になった時点で管理用 IPC / スクリプト |
| 4 | UNC とドライブ letter の **正規化** | 初版は不要。相対パス化で十分な場合が多い |
| 5 | `format.xlsx` の **自動シード（T-2）** を入れるか | 優先 1→2 解決＋手動配置で可。UX 向上なら DL 時コピーを実装 |
