...
**5-B（BOM CSV 取込・Rev・非表示）【計画・一部プロトタイプ実装済み・SW 形式は未対応】**

> **2026-05-25**: 簡易 CSV の取込・Rev・非表示等は **プロトタイプ実装済み**。**標準8列**（親品番・レベル）・`-` 埋め・ツリー維持ソートは **要件確定のみ（未実装）**。
...
- [ ] 再取込: **品番+Rev で更新** と **全置換** を選択可能。リピート品は同一品番+Rev を **流用**（§8.5.13.4）。
...
#### 8.5.15 BOM Rev 差分表示（設計変更の見える化）【5-F・IPC 実装済み／一覧バッジ・専用ページは §8.5.17.2】
